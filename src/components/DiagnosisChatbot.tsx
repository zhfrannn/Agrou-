...
import { X, Bot, CheckCircle2, Package, Plus, Search, HelpCircle, ShoppingCart, MessageCircle, Send, Star, ArrowRight } from "lucide-react";
...
export default function DiagnosisChatbot({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <motion.div 
        ...
        className="bg-white w-full max-w-[1100px] rounded-3xl shadow-2xl overflow-hidden flex h-[85vh] max-h-[680px]"
      >
        {/* ═══ LEFT SIDEBAR - CHAT CONTEXT ═══ */}
        <div className="w-[240px] bg-gradient-to-b from-[#2D6A4F] to-[#1B4332] text-white p-5 flex flex-col hidden lg:flex">
          <div className="mb-5">
            <div className="bg-white/20 backdrop-blur-sm p-2.5 rounded-xl mb-2.5 flex items-center gap-2.5">
              ...
              <div className="flex-1">
                <h3 className="font-display font-bold text-sm">Asisten Agrou</h3>
          ...
          <div className="bg-white border-b border-gray-100 px-5 py-3 shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-display font-bold text-gray-900 text-sm">Diagnosis Lahan</h3>
            ...
            <div className="flex items-center gap-1.5 mt-3">
              <div className={`flex-1 h-0.5 rounded-full transition-all ${step >= 1 ? 'bg-[#2D6A4F]' : 'bg-gray-200'}`}></div>
              <div className={`flex-1 h-0.5 rounded-full transition-all ${step >= 2 ? 'bg-[#2D6A4F]' : 'bg-gray-200'}`}></div>
              <div className={`flex-1 h-0.5 rounded-full transition-all ${step >= 3 ? 'bg-[#2D6A4F]' : 'bg-gray-200'}`}></div>
            </div>
          </div>
                  ...
                  <div>
                    <div className="bg-white border border-gray-100 p-3 rounded-xl rounded-tl-sm shadow-sm max-w-md">
                      <p className="text-gray-800 text-sm leading-relaxed">
                        Halo! 👋 Saya asisten Agrou. Anda bisa chat dengan saya atau pilih komoditas di bawah untuk diagnosis cepat.
                      </p>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1 ml-1">Sekarang</p>
                  </div>
                </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-3 lg:grid-cols-4 gap-2">
                      {[
                        ...
                        { id: 'rumputlaut', name: 'Rumput Laut', emoji: '🌿' },
                        ...
                        >
                          ...
                          <span className="font-bold text-gray-700 text-[11px]">{item.name}</span>
                        </button>
                      ))}
                      <button className="bg-gray-50 border border-dashed border-gray-200 hover:border-gray-300 p-2.5 rounded-lg transition-all flex flex-col items-center justify-center gap-1">
                        ...
                        <span className="font-bold text-gray-500 text-[11px]">Lainnya</span>
                      </button>
                    </div>
                  </>
                )}
              ...
              >
                ...
                <div className="flex items-end justify-end gap-2.5">
                  <div>
                    <div className="bg-[#2D6A4F] text-white px-3 py-2 rounded-xl rounded-tr-sm shadow-sm">
                      <p className="font-semibold text-xs">{selectedKomoditas}</p>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1 mr-1 text-right">Sekarang</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    <div className="bg-white border border-gray-100 p-3 rounded-xl rounded-tl-sm shadow-sm max-w-md">
                      <p className="text-gray-800 text-sm leading-relaxed">
                        Gejala apa yang terlihat? <span className="text-gray-400 font-normal text-xs">(bisa pilih lebih dari satu)</span>
                      </p>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1 ml-1">Sekarang</p>
                  </div>
                ...
                <div className="grid grid-cols-3 lg:grid-cols-4 gap-2">
                  {[
                    { id: 'kuning', label: 'Daun menguning', img: 'https://images.unsplash.com/photo-1530836369250-ef72a3f5c122?auto=format&fit=crop&q=80&w=300' },
                    { id: 'bercak', label: 'Bercak coklat', img: 'https://images.unsplash.com/photo-1588612197682-1dddcad05eb7?auto=format&fit=crop&q=80&w=300' },
                    { id: 'rebah', label: 'Batang rebah', img: 'https://images.unsplash.com/photo-1595841696677-6489ff3f8cd1?auto=format&fit=crop&q=80&w=300' },
                    { id: 'wereng', label: 'Ada hama', img: 'https://images.unsplash.com/photo-1590682680695-43b964a3ae17?auto=format&fit=crop&q=80&w=300' },
                    { id: 'kosong', label: 'Bulir kosong', img: 'https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&q=80&w=300' },
                    { id: 'busuk', label: 'Akar busuk', img: 'https://images.unsplash.com/photo-1628186100298-b8058204b7b6?auto=format&fit=crop&q=80&w=300' },
                  ].map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative rounded-lg overflow-hidden border transition-all shadow-sm group ${
                          isSelected ? 'border-[#2D6A4F] ring-1 ring-[#2D6A4F]/30 shadow-md' : 'border-gray-200 hover:border-[#2D6A4F]/50'
                        }`}
                      >
                        ...
                        <div className={`p-1.5 text-[10px] font-bold transition-colors ${isSelected ? 'bg-[#2D6A4F] text-white' : 'bg-white text-gray-700'}`}>
                          {item.label}
                        </div>
                      </button>
                    );
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="flex items-end justify-end gap-2.5">
                  <div>
                    <div className="bg-[#2D6A4F] text-white px-3 py-2 rounded-xl rounded-tr-sm shadow-sm">
                      <p className="font-semibold text-xs">{selectedGejala.length} Gejala Terpilih</p>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-1 mr-1 text-right">Sekarang</p>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl shadow-md overflow-hidden">
                  ...
                  {/* Header */}
                  <div className="p-3 bg-gradient-to-br from-red-50 to-orange-50 border-b border-gray-100">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${diagnosis.severityColor}`}>
                          <HelpCircle size={10} />
                  ...
                  <div className="p-3">
                    <div className="relative bg-gradient-to-br from-[#2D6A4F] to-[#1B4332] rounded-lg p-3 text-white overflow-hidden shadow-md mb-3">
                      ...
                      <div className="flex items-center gap-2 mb-2 mt-1">
                        ...
                        <h5 className="font-display font-bold text-xs">{diagnosis.bundle.name}</h5>
                      </div>
                      
                      <ul className="space-y-1 mb-3">
                        {diagnosis.bundle.items.map((item, i) => (
                          <li key={i} className="flex items-start gap-1.5 text-white/90">
                            <CheckCircle2 size={11} className="text-[#74C69D] mt-0.5 shrink-0" />
                      ...
                      <div className="flex items-center justify-between bg-black/10 rounded-md p-2 backdrop-blur-sm border border-white/10">
                        <div>
                          <div className="font-display font-black text-[#FFD166] text-base">{diagnosis.bundle.price}</div>
                          <div className="text-[8px] font-bold text-[#74C69D] uppercase tracking-wider">{diagnosis.bundle.saving}</div>
                        </div>
        ...
        {/* Close chat messages area */}
          ...
          <div className="border-t border-gray-200 bg-white px-4 py-3 shrink-0">
            <div className="flex items-center gap-2">
              ...
              >
                <Send size={16} className={chatInput.trim() ? '' : 'opacity-50'} />
              </button>
            </div>
          </div>
        </div>
        ...
        {step === 3 && (
          <motion.div
            ...
            className="w-[260px] bg-gray-50 border-l border-gray-200 p-4 flex flex-col hidden lg:flex overflow-y-auto"
          >
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex-1 space-y-3">
              {diagnosis.satuan.map((prod, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-[#F77F00] font-bold text-xs">{prod.price}</p>
